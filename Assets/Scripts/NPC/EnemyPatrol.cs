...
public class EnemyPatrol : MonoBehaviour {
    ...
    // Variables para destino, �rea de lanzamiento y jugador
    [SerializeField] Transform destination, throwarea, player;
    // Duraci�n de las animaciones de lanzar y recoger
    [SerializeField] float pickduration;
    // Agente de navegaci�n
    ...
    private IEnumerator PickUp()
    {
        ...
        // Iniciar la animaci�n de recoger
        GetComponent<Animator>().SetTrigger("pick");
        yield return new WaitForSeconds(pickduration);

        isPicking = false;

        // Ir a la zona de lanzamiento si ha recogido m�s de 2 cestas
    ...
    private IEnumerator ThrowFor() {
        // + 1 porque es de tipo int y el m�ximo es exclusivo
        ...
        //Debug.LogError("n� objects- " + randNumbObjects);
        //Debug.Log("anim- " + randAnimSpeed);

        for (int i = 0; i < randNumbObjects; i++) {
            if (isThrowing) {

                // Iniciar la animaci�n de lanzar
                anim.SetTrigger("throwing");

                // Esperar a que la animaci�n de lanzamiento comience
    ...
    {
        // Obtener una posici�n aleatoria de una cesta
