...
public class Spline : MonoBehaviour {
    ...
    #if UNITY_EDITOR
    /*private void Update() {
        #region Ver Par�bola en update
        Vector3 startPoint = _start.position;
        Vector3 targetPosition = player.position + Random.insideUnitSphere * launchRadius;
        // Static y, z position
        targetPosition.y = player.position.y;
        if (xOffset) targetPosition.x = player.position.x + xzOffset;   // Variante para la escena de juego que est� rotada
    ...
    private void LaunchObject(GameObject objectPrefab) {
        ...
        // Calcula el punto medio para este lanzamiento espec�fico
